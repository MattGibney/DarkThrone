...
export default function BankDepositPage(props: BankDepositPageProps) {
  ...
  async function deposit(amount: number) {
    ...
    if (depositsRemaining < 1) {
      setInvalidMessages([t('bank.deposit.maxReached')]);
      return;
    }

    const maxDeposit = Math.floor(playerGold * 0.8);
    if (amount < 1) {
      setInvalidMessages([t('bank.deposit.minimum')]);
      return;
    }
    if (amount > maxDeposit) {
      setInvalidMessages([
        t('bank.deposit.maxPercent'),
        t('bank.deposit.maxAmount', { maxDeposit }),
      ]);
      return;
    }
        ...
        >
          ...
          <div className="flex flex-col gap-y-4">
            <div className="flex gap-x-4">
              <Button
                text={t('bank.deposit.depositMax')}
                variant="primary-outline"
                type="button"
                onClick={handleDepositMax}
              />
              <Button text={t('bank.deposit.submit')} type="submit" />
            </div>
