...
export default function WarHistoryView(props: WarHistoryViewProps) {
  ...
  const [attackingPlayer, setAttackingPlayer] = useState<PlayerObject | null>(null);
  const [defendingPlayer, setDefendingPlayer] = useState<PlayerObject | null>(null);
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      const attackingPlayerFetch = await props.client.players.fetchByID(historyFetch.data.attackerID);
      ...
      const defendingPlayerFetch = await props.client.players.fetchByID(historyFetch.data.defenderID);
      ...
      <div className='my-12 w-full max-w-2xl mx-auto rounded-xl'>
        <div className='bg-zinc-800/50 p-8 flex justify-center'>
          ...
          <div className='w-1/4'>
            ...
            <div className='text-center font-bold text-zinc-300 mt-2'>{attackingPlayer?.name}</div>
          </div>
          ...
          <div className='w-1/4'>
            ...
            <div className='text-center font-bold text-zinc-300 mt-2'>{defendingPlayer?.name}</div>
          </div>

        </div>
        <div className='flex flex-col items-center gap-y-3 bg-zinc-800 text-zinc-300 p-8'>
          <p><span className='text-white font-semibold'>{attackingPlayer?.name}</span> attacked <span className='text-white font-semibold'>{defendingPlayer?.name}</span></p>
          <p>{history?.attackTurnsUsed} attack turn(s) were used</p>
          <p><span className='text-white font-semibold'>{attackingPlayer?.name}</span> had a strength of {history?.attackerStrength}</p>
          {history?.defenderStrength !== undefined ? (
            <p><span className='text-white font-semibold'>{defendingPlayer?.name}</span> had a strength of {history?.defenderStrength}</p>
          ) : null}
          <p><span className='text-white font-semibold'>{history?.isAttackerVictor ? attackingPlayer?.name : defendingPlayer?.name}</span> was victorous</p>
        </div>
      </div>
    </main>
  );
